...
local SaveManager = {} do
    ...
    SaveManager.Parser = {
        ...
        Slider = {
            ...
            Load = function(idx, data)
                ...
                if object and object:Get() ~= tonumber(data.value) then
                    object:Set(tonumber(data.value))
                end
            end,
        },
        Dropdown = {
            ...
            Load = function(idx, data)
                local object = SaveManager.Library.Options[idx]
                if object and object:Get() ~= data.value then
                    object:Set(data.value)
                end
            end,
        },
        Input = {
            ...
            Load = function(idx, data)
                local object = SaveManager.Library.Options[idx]
                if object and object:Get() ~= data.text and type(data.text) == "string" then
    ...
    function SaveManager:Save(name)
        ...
        for idx, option in pairs(self.Library.Options) do
            ...
            if ok then
                table.insert(data.objects, { type = "Input", idx = idx, text = tostring(value) })
            end
        end
    ...
    function SaveManager:BuildConfigSection(tab)
        ...
        local section = tab:AddGroup({Name = "Configuration", Side = "Right", Icon = "folder-cog"})
        ...
        section:AddButton({
            Text = "Create config",
            Callback = function()
                local name = self.Library.Options.SaveManager_ConfigName:Get()

                if name:gsub(" ", "") == "" then
                    self.Library:Notify({Title = "Error", Description = "Invalid config name (empty)", Duration = 2})
                    return
                end

                local success, err = self:Save(name)
                if not success then
                    self.Library:Notify({Title = "Error", Description = "Failed to create config: " .. err, Duration = 3})
                    return
                end

                self.Library:Notify({Title = "Config", Description = "Created config: " .. name, Duration = 3})
        ...
        section:AddButton({
            Text = "Load config",
            Callback = function()
                local name = self.Library.Options.SaveManager_ConfigList:Get()

                local success, err = self:Load(name)
                if not success then
                    self.Library:Notify({Title = "Error", Description = "Failed to load config: " .. err, Duration = 3})
                    return
                end

                self.Library:Notify({Title = "Config", Description = "Loaded config: " .. name, Duration = 3})
            end
        })
        section:AddButton({
            ...
            Callback = function()
                ...
                local success, err = self:Save(name)
                if not success then
                    self.Library:Notify({Title = "Error", Description = "Failed to overwrite config: " .. err, Duration = 3})
                    return
                end

                self.Library:Notify({Title = "Config", Description = "Overwrote config: " .. name, Duration = 3})
            end
        })
        ...
        section:AddButton({
            ...
            Callback = function()
                ...
                if not success then
                    self.Library:Notify({Title = "Error", Description = "Failed to delete config: " .. err, Duration = 3})
                    return
                end

                self.Library:Notify({Title = "Config", Description = "Deleted config: " .. name, Duration = 3})
                self.Library.Options.SaveManager_ConfigList:SetValues(self:RefreshConfigList())
                self.Library.Options.SaveManager_ConfigList:SetValue(nil)
            end
        })
        ...
        section:AddButton({
            ...
            Callback = function()
                ...
                if not success then
                    self.Library:Notify({Title = "Error", Description = "Failed to set autoload config: " .. err, Duration = 3})
                    return
                end

                self.Library:Notify({Title = "Config", Description = "Set " .. name .. " to auto load", Duration = 3})
        ...
        section:AddButton({
            ...
            Callback = function()
                ...
                self.Library:Notify({Title = "Config", Description = "Set autoload to none", Duration = 3})
                self.AutoloadConfigLabel:SetText("Current autoload config: none")
            end
        })

        self.AutoloadConfigLabel = section:AddLabel({Name = "Current autoload config: " .. self:GetAutoloadConfig()})
